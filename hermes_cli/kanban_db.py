...
def add_notify_sub(
    ...
    *,
    ...
    ``chat_type`` is the session's origin chat type ("dm", "group", etc.)
    persisted so the wake injection can reconstruct the *correct* SessionSource
    instead of defaulting to "group". Defaults to "group" for legacy callers
    and CLI-only subscriptions that lack a session source, preserving the
    prior behavior on rows where it is unknown.
    """
    resolved_chat_type = chat_type or "group"
    now = int(time.time())
    with write_txn(conn):
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (task_id, platform, chat_id, thread_id or "", user_id, notifier_profile, resolved_chat_type, now),
        )
        if notifier_profile:
